...
<div class="container">
    <h3 style="margin-top: 12px;" class="alert alert-success">Laravel Ajax CRUD Application
     </h3><br>
     {{-- IsMitesh has than create a post --}}
          ...
          <table id="laravel_crud" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                <tr id="row_{{$post->id}}">
                   ...
                   <td>{{ $post->candidate->first_name. " ".$post->candidate->last_name }}</td>
        ...
        <div class="modal-body">
            <form name="userForm" class="form-horizontal">
               ...
               <div class="form-group">
                <label for="candidate" class="col-sm-2">Candidate</label>
                <div class="col-sm-12">
                <select class="form-control" id="candidate" name="candidate">
                  <option value="">Select candidate</option>
                  @foreach($candidates as $candidate)
                  <option value="{{ $candidate->id }}" {{ old('candidate') == $candidate->id ? 'selected' : '' }}>{{ $candidate->first_name. " ". $candidate->last_name }}</option>
                  @endforeach
                </select>
                <span id="candidateError" class="alert-message"></span>
                </div>
                </div>
                <div class="form-group">
...
<script>
    ...
    $.ajax({
      ...
      success: function(response) {
          if(response) {
            ...
            $("#candidate").val(response.candidate.id);
            $('#post-modal').modal('show');
          }
      }
    });
  ...
  function createPost() {
    ...
    var employee = $("#candidate option:selected" ).text();
    ...
    let _token   = $('meta[name="csrf-token"]').attr('content');
      ...
      $.ajax({
        ...
        success: function(response) {
            if(response.code == 200) {
              if(id != ""){
                ...
                $("#row_"+id+" td:nth-child(4)").html(employee);
              } else {
                $('table tbody').prepend('<tr id="row_'+response.data.id+'"><td>'+response.data.id+'</td><td>'+response.data.title+'</td><td>'+response.data.description+'</td><td>'+employee+'</td><td><a href="javascript:void(0)" data-id="'+response.data.id+'" onclick="editPost(event.target)" class="btn btn-info">Edit</a></td><td><a href="javascript:void(0)" data-id="'+response.data.id+'" class="btn btn-danger" onclick="deletePost(event.target)">Delete</a></td></tr>');
              }
              $('#title').val('');
              $('#description').val('');
        ...
        error: function(response) {
          $('#candidateError').text(response.responseJSON.errors.candidate);
